...
class EditBlurb extends Component {
  ...
  handleDelete(e) {
    ...
    fetch(`/api/blurbs/${user._id}/${this.state.members.id}`, {
      ...
      .then(res => {
        if (!res.success)
          this.setState({ error: res.error, alertText: "Error" });
        else
          this.setState({
            members: {
              ...this.state.members,
  ...
  getAlert() {
    if (this.state.alertText === "Updated") {
      return (
        <Alert color="success">
          Blurb was successfully updated! <Link to="/">Click here</Link> to see
          your changes, or you can continue editing below.
        </Alert>
      );
    } else if (this.state.alertText === "Error") {
      return <Alert color="danger">Error: {this.state.error}</Alert>;
    } else if (this.state.alertText === "Deleted") {
      return (
        <Alert color="success">
          Blurb was successfully deleted! <Link to="/">Click here</Link> to see
